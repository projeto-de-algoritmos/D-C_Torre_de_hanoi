...
class Hanoi:
    ...
    def __hanoi(self, levels, start, target, n):
        if n == 1:
            ...
            aux = 3 - start - target 
            for i in self.__hanoi(levels, start, aux, n-1): yield i
            for i in self.__hanoi(levels, start, target, 1): yield i
            for i in self.__hanoi(levels, aux, target, n-1): yield i

    def __pyramid(self, levels, start_x, start_y, level_height, screen):
        for i, levelwidth in enumerate(levels):
            pygame.draw.rect(screen, (238-levelwidth, 100-levelwidth, 123-levelwidth), (start_x + (self.space_per_level - levelwidth)/2 , start_y - level_height * i, levelwidth, level_height))
        ...
        while True:
            for position in positions:
                ...
                for i, pile in enumerate(position):
                    self.__pyramid(pile, 10 + self.space_per_level*i, 300, self.level_height, screen)
